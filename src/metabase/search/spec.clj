...
(defn- find-fields-expr
  ...
  ([acc expr]
   (cond
     (keyword? expr)
     (conj! acc (find-fields-kw expr))

     (and (vector? expr) (> (count expr) 1))
     (reduce find-fields-expr2 acc (subvec expr 1))

     (and (map? expr) (:fields expr))
     (reduce-kv #(find-fields-expr2 %1 %3) acc (:fields expr))
...
(defn- find-fields
  ...
  (u/group-by #(nth % 0) #(nth % 1) conj #{}
              (as-> (transient []) acc
                ...
                (reduce find-fields-expr acc (:where spec))
                (persistent! acc))))
